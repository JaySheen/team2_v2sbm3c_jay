...
CREATE SEQUENCE brnotice_seq
  ...
  INSERT INTO brnotice(notice_no, adminno, notice_name, notice_post, notice_view, recom, passwd, notice_word, notice_crtime, notice_mdtime)
VALUES (brnotice_seq.nextval, 3, '빵파고에 오신것을 환영합니다!!', '빵파고는 사람들이 서울내에 다양한 빵 맛집을 추천해주는 카페입니다!! ', 0, 0, 1234, '#공지사항', sysdate, sysdate);

-- R(List)
SELECT notice_no, adminno, notice_name, notice_post, notice_view, recom, passwd, notice_word, notice_crtime, notice_mdtime
FROM brnotice
ORDER BY notice_no;
...
SELECT COUNT(*) as cnt
    FROM brnotice
    WHERE   (notice_name LIKE '%빵파고%' OR notice_post LIKE '%빵파고%' OR notice_word LIKE '%#공지%')
...
SELECT COUNT(*) as cnt 
    FROM brnotice
    WHERE notice_no=2 AND passwd=1234
