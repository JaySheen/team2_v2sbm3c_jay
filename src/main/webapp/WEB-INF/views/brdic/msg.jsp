...
<DIV class='title_line'> 🍞빵 백과사전🍞 > 알림</DIV>
    ...
    <UL>
      <c:choose>
        ...
        <c:when test="${param.code == 'create_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">새로운 빵 백과사전글을 등록했습니다.</span>
          </LI>  
          <LI class='li_none'>
            <button type='button' 
                         onclick="location.href='./create.do'"
                         class="btn btn-default">새로운 빵 백과사전글 등록</button>
          </LI> 
        </c:when>
        <c:when test="${param.code == 'create_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">새로운 빵 백과사전글 등록에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        <c:when test="${param.code == 'update_text_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">빵 백과사전글을 수정했습니다.</span>
          </LI>                                                                      
        </c:when>
        <c:when test="${param.code == 'update_text_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">빵 백과사전글 수정에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        <c:when test="${param.code == 'update_file_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">빵 백과사전글 사진이 새로 업로드 됐습니다.</span>
          </LI>                                                                      
        </c:when>
        <c:when test="${param.code == 'update_file_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">빵 백과사전글 사진 수정에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        <c:when test="${param.code == 'delete_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">빵 백과사전글 삭제에 성공했습니다.</span>
          </LI>                                                                      
        </c:when>        
        <c:when test="${code == 'delete_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">빵 백과사전글 삭제에 실패했습니다.</span>
          </LI>                                                                      
        </c:when> 
        <c:otherwise>
      ...
      <LI class='li_none'>
        ...
        <button type='button' onclick="location.href='./list_by_dic_no_search_paging.do'" class="btn btn-primary">목록</button>
